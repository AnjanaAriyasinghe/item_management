...
@section('title', 'Cheque Books Details')
@section('breadcrumb-item', 'Admin')
@section('breadcrumb-item-active', 'Cheque Books Details')
@section('content')
<style>
    .card-body-1 {
...
<div class="modal fade" id="rejectModel" tabindex="-1" role="dialog" aria-labelledby="rejectModelLabel" aria-hidden="true">
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="submitFormBtnReject" method="POST" action="{{route('admin.cheque.cancelled')}}">
                    @csrf
                    <input type="hidden" name="id" id="data_value_reject">
                    <div class="form-group row g-4">
                        <div class="col-md-12">
                            <div class="form-group form-floating">
                                <textarea class="form-control" id="cancelled_comment" style="height: 100px" name="cancelled_comment"></textarea>
                                <label for="floatingTextarea">Comment</label>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(function() {
        var table = $('.data-table').DataTable({
            ...
            , serverSide: true
            , ajax: {
                url: "{{ route('admin.cheque_books.details') }}"
                , data: function(d) {
            ...
            , columns: [{
                ...
                , {
                    data: 'cheque_book.nikname'
                    , name: 'cheque_book.nikname'
                }
    ...
    $(document).on('click', '.btnView', function() {
        ...
        $.ajax({
            ...
            , success: function(response) {
                ...
                if (response.book_details.payment?.vendor !== undefined) {
                    $('#view_vendor_account').text(response.book_details.payment.expense.vendor_account?response.book_details.payment.expense.vendor_account.mobile:"-");
                } else {
                    $('#view_vendor_account').text('');
                }
                $('#layout_cheque_no').text(response.book_details.cheque_number);
                $('#view_cheque_no').text(response.book_details.cheque_number);
                $('#view_referance_no').text(response.book_details.referance_no);
