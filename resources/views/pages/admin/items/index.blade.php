    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="submitForm" method="POST" action="{{ route('admin.items.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="row g-3">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-12">
                            <div class="form-floating">
                                <textarea class="form-control" id="item_description" name="item_description" placeholder=" " style="height:100px;"></textarea>
                                <label for="item_description">Item Description</label>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-floating">
                                <input class="form-control" type="number" id="unit_price" name="unit_price" placeholder=" " step="0.01" min="0">
                        ...
                        <div class="col-md-12">
                            ...
                            <div id="photo_preview_wrapper" class="mt-2" style="display:none;">
                                <img id="photo_preview" src="" alt="Current Photo" style="max-height:120px;border-radius:8px;border:1px solid #dee2e6;">
                                <p class="text-muted small mt-1 mb-0">Current photo — upload a new one to replace it.</p>
                            </div>
                        </div>
                    </div>
                </form>
    ...
    <div class="col-sm-12">
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                @can('admin-common-items-create')
                <button type="button" class="btn btn-primary btn-sm btn-add-new" data-bs-toggle="modal" data-bs-target="#createModel">
...
$(function () {
    ...
    var table = $('#itemsTable').DataTable({
        ...
        columns: [
            ...
            { data: 'item_description',  name: 'item_description',  orderable: false },
    ...
    $(document).on('click', '.btnView', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#view_unit_price').text(item.unit_price !== null ? parseFloat(item.unit_price).toFixed(2) : '—');
                $('#view_item_description').text(item.item_description || '—');
                $('#view_created_by').text(response.created_name || '—');
                $('#view_created_at').text(item.created_at ? new Date(item.created_at).toLocaleDateString('en-US') : '—');

                // Photo
                if (response.photo_url) {
                    $('#view_photo').attr('src', response.photo_url);
    ...
    $(document).on('click', '.btnEdit', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#unit_price').val(response.item.unit_price !== null ? parseFloat(response.item.unit_price).toFixed(2) : '');
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                error: function (xhr) {
                    Swal.fire({ icon: 'error', title: 'Error', text: xhr.responseJSON?.message || 'Could not delete.' });
                }
            });
        }
    });
